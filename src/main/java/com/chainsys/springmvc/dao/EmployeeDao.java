...
//		int rows ;
		...
		try {
			...
			ps.setDate(5, convertTosqlDate(newemp.getHireDate()));
...
// for updating all the columns of the table
	public static int updateEmployee(Employee newemp) {
		...
		try {
			...
			ps.setInt(7, newemp.getEmpId());
			ps.setString(2, newemp.getLast_name());
			ps.setString(3, newemp.getEmail());
			// convert java.util.Date to java.sql.date
			ps.setDate(4, convertTosqlDate(newemp.getHireDate()));
			ps.setString(5, newemp.getJobId());
		...
		try {
			...
			emp = new Employee();
			   if(rs.next()) {
		          ...
		          emp.setHireDate(date);//ok sir
		          emp.setJobId(rs.getString(6));
		          emp.setSalary(rs.getLong(7));
		        }
		}catch(SQLException e) {
			e.printStackTrace();
		}finally {
